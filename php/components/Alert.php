...
class Alert
{
    ...
    /** 
     * @return boolean
     * Return true if the alert should be activated, acording to alert's rule and value
    ...
    public function isActive($metricLogs)
    {
       ...
       //echo "<br><br>".json_encode($metricLogs);
        if(!$this->alert)
            return false;
        usort($metricLogs, array($this, "compareTimeStampDesc")); //Last log is the first element
     ...
     * 
     * @return number
     * Compare two metric logs in order to ascending sort an array
     */
     ...
     *
     * @return number
     * Compare two metric logs by therir time stamp in order to descending sort an array
     */
    ...
    /** 
     * @return number
     * Compare two metric logs by their value
     */
